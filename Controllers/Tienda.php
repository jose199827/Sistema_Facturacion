...
class Tienda extends Controllers
{
  ...
  public function Categoria($params)
  {
    if (empty($params)) {
      ...
      if (empty($idcategoria) or empty($ruta)) {
        header('Location: ' . Base_URL());
      }
      $pagina = 1;
  ...
  public function search()
  {
    ...
    /* dep($data['productos']);
        exit(); */
    $data['page_tag'] = NOMBRE_EMPRESA;
    $data['page_title'] = "Resultado de busqueda <a href='#' class='filter-link filter-link-active'>" . $busqueda . "</a>";
    $data['page_name'] = "tienda";
    $data['pagina'] = $pagina;
    $data['totalPaginas'] = $total_pagina;
  ...
  {
    if ($_POST) {
      $nombreUsuario = ucwords(strtolower(strClean($_POST['nombreSubcripcion'])));
      $emailUsuario = strtolower(strClean($_POST['emailSubcripcion']));
      $suscripcion = $this->setSuscripcion($nombreUsuario, $emailUsuario);
      /* dep($suscripcion); */
      if ($suscripcion > 0) {
  ...
  public function contancto()
  {
    /* dep($_SERVER);
        exit(); */
    if ($_POST) {
      $nombreUsuario = ucwords(strtolower(strClean($_POST['nombrecontancto'])));
      $emailUsuario = strtolower(strClean($_POST['emailcontancto']));
      $msgcontancto = strClean($_POST['msgcontancto']);
      ...
      $contancto = $this->setContancto($nombreUsuario, $emailUsuario, $msgcontancto, $useragent, $ip, $dispositivo);
      /*  dep($contancto);
            exit(); */
      if ($contancto > 0) {
        $arrResponse = array("status" => true, "msg" => 'Gracias por tu comentario.');
        //Enviar el correo
        $dataUsuario = array(
          'asunto' => 'Nueva usuario en comentar',
          'email' => EMAIL_SUSCRIPCION,
          'nombreUsuario' => $nombreUsuario,
          'emailUsuario' => $emailUsuario,
          'mensaje' => $msgcontancto
        );
        sendEmail($dataUsuario, 'email_contacto');
      } else {
        $arrResponse = array("status" => false, "msg" => 'No fue posible enviar el mensaje.');
      }
      echo json_encode($arrResponse, JSON_UNESCAPED_UNICODE);
    }
    die();
