...
trait TCliente
{
  ...
  public function setSuscripcion(string $nombreUsuario, string $emailUsuario)
  {
    $this->con = new Mysql();
    $return = "";
    $sql = "SELECT idsuscripcion,nombre,email,datedreated FROM suscripciones WHERE email='{$emailUsuario}'";
    $requestSuscripcion = $this->con->selectAll($sql);
    if (empty($requestSuscripcion)) {
      $query_insert = "INSERT INTO `suscripciones` (`nombre`,`email`)
                          VALUES (?,?);";
      $arrData = array($nombreUsuario, $emailUsuario);
      $request_insert = $this->con->insert($query_insert, $arrData);
      return $request_insert;
    } else {
      $return = false;
  ...
  public function setContancto(string $nombreUsuario, string $emailUsuario, string $msgcontancto, string $useragent, string $ip, string $dispositivo)
  {
    $this->con = new Mysql();
    $nombreUsuario = $nombreUsuario != "" ? $nombreUsuario : "";
    $emailUsuario = $emailUsuario != "" ? $emailUsuario : "";
    $msgcontancto = $msgcontancto != "" ? $msgcontancto : "";
    $useragent = $useragent != "" ? $useragent : "";
    $ip = $ip != "" ? $ip : "";
    $dispositivo = $dispositivo != "" ? $dispositivo : "";
    $query_insert = "INSERT INTO `contacto`(`nombre`, `email`, `mensaje`, `dispositivo`, `ip`, `useragent`) 
                     VALUES (?,?,?,?,?,?)";
    $arrData = array($nombreUsuario, $emailUsuario, $msgcontancto, $dispositivo, $ip, $useragent);
    $request_insert = $this->con->insert($query_insert, $arrData);
    return $request_insert;
  }
}
